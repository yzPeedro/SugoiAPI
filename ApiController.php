...
{   
    ...
    public function searchAnime( $anime ) {
        try {
            ...
            foreach ( $links as $links_format ) {
                if ( get_headers($links_format)[2] == "Content-Type: video/mp4" || get_headers($links_format)[3] == "Content-Type: video/mp4" ) {
                    array_push($arr_search, ["anime" => $anime, "audio" => (strpos($links_format, "dublado")) ? "dublado" : "legendado", "link" => $links_format,"status" => "HTTP/1.1 200" ]);
                    continue;
                } else if ( $links_format == end($links) ) {
                    if ( !empty($arr_search) ) {
                        http_response_code(200);
                        return json_encode( $arr_search );
                    } else {
                        http_response_code(404);
                        return json_encode(["error" => "Not Found", "status" => "HTTP/1.1 404"]);            
                    }
